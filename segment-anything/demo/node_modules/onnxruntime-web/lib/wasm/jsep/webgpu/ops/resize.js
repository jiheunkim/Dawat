...
exports.parseResizeAttributes = exports.resize = exports.createResizeProgramInfoLoader = void 0;
...
const getOriginalCoordinateFromResizedCoordinate = (coordinateTransferMode) => 'fn getOriginalCoordinateFromResizedCoordinate(xResized: f32, xScale: f32, lengthResized: f32,\
    ...
    '}';
const getNearestPixelFromOriginal = (nearestMode, opsetVersion) => 'fn getNearestPixelFromOriginal(xOriginal: f32, isDownSample: bool) -> f32 {' + (() => {
    switch (nearestMode) {
...
const adjustOutputShape = (inputShape, outputShape, scales, attributes) => {
    const scaleInPolicy = (() => {
...
const calculateOriginalIndicesFromOutputIndices = (output, inputShape, outputShape, scales, roi) => `
    fn calculateOriginalIndicesFromOutputIndices(outputIndices: ${output.type.indices}) -> array<f32, ${outputShape.length}> {
...
const calculateInputIndicesFromOutputIndices = (input, output, inputShape, outputShape, scales, roi, useExtrapolation) => `
...
const bilinearInterpolation = (input, output, inputShape, outputShape, scales, useExtrapolation, extrapolationValue) => {
    const [batchIdx, heightIdx, widthIdx, channelIdx] = inputShape.length === 2 ? [-1, 0, 1, -1] : (scales[1] === 1.0 ? [0, 2, 3, 1] : [0, 1, 2, 3]);
    return `
    ...
    fn bilinearInterpolation(outputIndices: ${output.type.indices}) -> f32 {
      ...
      if (${useExtrapolation} && (row < 0 || row > (${inputShape[heightIdx]} - 1) || col < 0 || col > ${inputShape[widthIdx]} - 1)) {
...
const bicubicInterpolation = (input, output, inputShape, outputShape, scales, roi, cubicCoeffA, useExtrapolation, extrapolationValue, excludeOutside) => {
    ...
    const createCubicInterpolationFunction = (idx) => {
        ...
        return `
      fn ${direction}CubicInterpolation(inputIndices: ${input.type.indices}, outputIndices: ${output.type.indices}) -> f32 {
...
const createResizeProgramInfo = (metadata, inputTensor, attributes, opsetVersion, scalesInput, sizes, roiInput) => {
    ...
    const output = (0, common_1.outputVariable)('output', inputTensor.dataType, outputShape);
    const input = (0, common_1.inputVariable)('input', inputTensor.dataType, inputShape);
      ...
      ${(() => {
        switch (attributes.mode) {
            case 'nearest':
              ...
              ${calculateInputIndicesFromOutputIndices(input, output, inputShape, outputShape, scales, roi, useExtrapolation)};
              `;
            case 'linear':
                return `
              ${calculateOriginalIndicesFromOutputIndices(output, inputShape, outputShape, scales, roi)};
              ${bilinearInterpolation(input, output, inputShape, outputShape, scales, useExtrapolation, attributes.extrapolationValue)};
              `;
            case 'cubic':
                return `
            ${bicubicInterpolation(input, output, inputShape, outputShape, scales, roi, attributes.cubicCoeffA, useExtrapolation, attributes.extrapolationValue, attributes.excludeOutside)};
            `;
            default:
...
const createResizeProgramInfoLoader = (input, attributes, opsetVersion, scales, sizes, roi) => {
    const metadata = {
        name: 'Resize',
...
exports.createResizeProgramInfoLoader = createResizeProgramInfoLoader;
...
const resize = (context, attributes) => {
    const scales = [];
    const sizes = [];
    const roi = [];
    const opsetVersion = getOpsetVersionFromCustomDataBuffer(context);
    validateInputs(context.inputs, attributes, opsetVersion, scales, sizes, roi);
    context.compute((0, exports.createResizeProgramInfoLoader)(context.inputs[0], attributes, opsetVersion, scales, sizes, roi), { inputs: [0] });
};
exports.resize = resize;
const parseResizeAttributes = (attributes) => {
...
exports.parseResizeAttributes = parseResizeAttributes;
