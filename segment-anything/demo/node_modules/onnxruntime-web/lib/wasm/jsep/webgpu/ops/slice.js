...
exports.parseSliceAttributes = exports.slice = void 0;
...
const validateInputs = (inputs, attributes) => {
    ...
    inputs.slice(1).forEach((_, idx) => {
        if (inputs[idx + 1].dataType !== 6 /* DataType.int32 */ && inputs[idx + 1].dataType !== 7 /* DataType.int64 */) {
            throw new Error(`Input ${idx} must be an array of int32 or int64`);
        }
    });
};
const readInput = (inputs, idx) => {
    const input = [];
    if (inputs.length > idx) {
        if (inputs[idx].dataType === 7 /* DataType.int64 */) {
            inputs[idx].getBigInt64Array().forEach(v => input.push(Number(v)));
        }
        else if (inputs[idx].dataType === 6 /* DataType.int32 */) {
...
const createSliceAttributesFromInputs = (inputs, attributes) => {
    if (inputs.length > 1) {
...
const fixStartEndValues = (value, index, inputShape, axes, steps) => {
    let newValue = value;
    if (value < 0) {
...
const calculateInputIndicesImpl = (input, output, inputShape, outputShape) => `fn calculateInputIndices(outputIndices: ${output.type.indices}) -> ${input.type.indices} {
...
const createSliceProgramInfo = (metadata, inputs, attributes) => {
    const inputShape = inputs[0].dims;
    const inputSize = util_1.ShapeUtil.size(inputShape);
    const axes = (attributes.axes.length > 0) ? util_1.ShapeUtil.normalizeAxes(attributes.axes, inputShape.length) :
    ...
    const outputTensorInfo = { dims: outputShape, dataType: inputs[0].dataType, gpuDataType: types_1.GpuDataType.default };
    const output = (0, common_1.outputVariable)('output', inputs[0].dataType, outputShape);
    const input = (0, common_1.inputVariable)('input', inputs[0].dataType, inputShape);
...
const createSliceProgramInfoLoader = (inputs, attributes) => {
...
const slice = (context, attributes) => {
    ...
    if (util_1.ShapeUtil.size(program.outputs[0].dims) > 0) {
...
exports.slice = slice;
const parseSliceAttributes = (attributes) => {
