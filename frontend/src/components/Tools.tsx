...
import { postAutoAnnotReq } from "../api/dawatAxios";
...
function Tools() {
  ...
  return (
    <>
      ...
      <div
        className={`absolute ${
          loacation.pathname !== "/tool/annotation" && "hidden"
        }`}
        style={{
            ...
            onClick={async () => {
              ...
              try {
                const fetchedMaskInfo = await postAutoAnnotReq(
                  docList!!.find((item) => item.src === image!!.src)!!.file_name
                );
                if (fetchedMaskInfo) {
